...
if (adminUser && adminUser.userLocations.length > 0) {
  const approvePermission = await prisma.permission.findUnique({
    where: {
      userLocationId_name: {
        userLocationId: adminUser.userLocations[0].id,
  ...
  if (!approvePermission) {
    ...
    console.log('Added content.approve permission');
  }
}
